...
### Version 2.1 — IstioRevisionTag-Based Namespace Labeling (Label Once, Upgrade Forever)

**Environment:** Production OpenShift Cluster — `lab.test.local` (On-Premises)
...
9. [Phase 5 — Migrate Workload Namespaces (Batch Label + Rollout)](#9-phase-5--migrate-workload-namespaces-batch-label--rollout)
...
### 7.4 Capture the Active Revision Name (Optional — for debugging)
...
> **Note:** You no longer need this variable for namespace labeling (Phase 5) or gateway labeling (Phase 4). Those use `istio.io/rev=default` (the tag name). Keep capturing this value only for debugging — e.g., to check which revision a specific pod is connected to via `istioctl ps`.

### 7.6 Create the IstioRevisionTag (Critical — enables tag-based labeling)

Every namespace label uses `istio.io/rev=default` (the tag name), not the concrete revision. This tag must exist before Phase 4/5 can work. It is created manually here in the migration guide because readers follow this guide independently from the upgrade guide.

The tag points to `Istio/basic` and auto-follows the operator's `activeRevisionName` as it changes during upgrades.
...
spec:
  targetRef:
    ...
    kind: Istio
    name: basic
    revision: ""   # empty — auto-resolves to activeRevisionName
```
...
Verify the tag resolved correctly:

```bash
oc get istiorevisiontag default -n istio-system -o jsonpath='{.status}'
# Expected output: {"conditions":[...],"observedGeneration":1,"phase":"Accepted","revision":"basic-v1-24-3"}
# The "revision" field shows which concrete revision the tag points to
```

> **Why this matters:** Without this tag, `istio.io/rev=default` in namespace labels is meaningless — the injector won't find a tag named `default` and injection will fail. This is the bridge between the abstract tag name in labels and the concrete revision running in `istio-system`.

### 7.7 Apply mTLS Strict Mode
...
- Enable sidecar injection from the **OSSM 3** control plane via the IstioRevisionTag (`istio.io/rev=default`)
- Prevent the **OSSM 2** injection webhook from interfering (`maistra.io/ignore-namespace`)
- Match the `discoverySelectors` defined in the Istio resource (`service-mesh=enabled`)

> **Why use `default` instead of `${ACTIVE_REVISION}`:**
> We use the IstioRevisionTag name `default` (created in Phase 0 of the upgrade guide) instead of the concrete revision name. The tag auto-follows the Istio CR's active revision, so:
> - During OSSM 2 → 3 migration: `default` points to `basic-v1-24-3`
> - During 3.0 → 3.1 → 3.2 → 3.3 upgrades: `default` auto-follows to each new revision
> - **You only ever label once, and never touch namespace labels again.**
...
> - `istio.io/rev=default` — any new pod created in `istio-system` gets its sidecar proxy from the OSSM 3 control plane (via the tag, which auto-follows the active revision)
...
> - `service-mesh=enabled` — matches the `discoverySelectors` in the Istio resource, ensuring the gateway namespace is discovered by the OSSM 3 control plane
...
```
OSSM 2 時期                          OSSM 2→3 遷移中期
┌──────────────────┐                 ┌──────────────────┐
│ istio-system      │                 │ istio-system      │
│                    │                 │                    │
│ Service (selector: │                 │ Service (selector: │
│  istio=ingressgateway)              │  istio=ingressgateway) ← 沒變
│         ▲         │                 │         ▲         │
│         │         │                 │    ┌────┴────┐    │
│ ┌───────┴────┐    │                 │ ┌──┴───┐  ┌──┴───┐│
│ │舊 Deployment│    │                 │ │舊    │  │新    ││
...
> **Note on `istio.io/rev` label:** The canary deployment uses `istio.io/rev=default` (the tag name) instead of the concrete revision name. This ensures the canary always connects to whichever revision the tag currently points to, and survives future upgrades (3.0 → 3.1 → 3.2 → 3.3) without any label changes.
  ...
  template:
    metadata:
      ...
      labels:
        ...
        istio.io/rev: default              # Tag name — auto-follows active revision
    spec:
...
> - `istio.io/rev: default` — uses the tag name, not the concrete revision name. The tag auto-follows the active revision, so this survives all future upgrades without changes
...
If you do not need fine-grained traffic control, restart the existing gateway deployment. The sidecar injector will re-inject it with the OSSM 3 proxy (because the namespace is now labelled with `istio.io/rev=default`).
...
# (connected to istiod-basic-v1-24-3 or whichever revision the tag points to)
...
Then apply the same Phase 4 canary/in-place migration: since `istio-system` namespace is already labelled with `istio.io/rev=default` and `maistra.io/ignore-namespace="true"` from Section 8.3, simply restart the egress gateway:
...
This phase migrates each workload namespace from the OSSM 2 control plane to the OSSM 3 control plane. Both control planes share the same root certificate, so mTLS communication between workloads on different control planes continues to work throughout the migration.

> **Key insight — label once, upgrade forever:**
> Every namespace uses `istio.io/rev=default` (the tag name), not a concrete revision name. The tag auto-follows the Istio CR's active revision. This means:
> - **During OSSM 2 → 3 migration:** label all namespaces once, then rollout restart
> - **During 3.0 → 3.1 → 3.2 → 3.3 upgrades:** NO label changes needed. Just rollout restart after each version bump
> - The `maistra.io/ignore-namespace="true"` label stays until OSSM 2 is fully removed (Phase 7)
...
### 9.1 Batch Label All Mesh Namespaces

Apply the injection labels to **all** mesh member namespaces at once. This replaces the old per-namespace approach:

```bash
# All namespaces that were in the ServiceMeshMemberRoll
MESH_NAMESPACES="devops project-01 project-02 project-03 project-04"
...
> - `istio.io/rev=default` — directs new pods to the OSSM 3 proxy via the IstioRevisionTag (auto-follows active revision)
> - `maistra.io/ignore-namespace="true"` — prevents the OSSM 2 injection webhook from injecting proxies. **Keep this label until Phase 7** when OSSM 2 is fully removed
> - `service-mesh=enabled` — matches the `discoverySelectors` in the Istio resource, ensuring workloads are discovered by OSSM 3
> - `istio-injection-` — removes the old OSSM 2 namespace-level injection label if present

### 9.2 Migrate Namespaces One at a Time

For each namespace, rollout restart the deployments, verify, and test connectivity:

#### project-01 (hello-world1, hello-world2)

```bash
oc rollout restart deployment -n project-01
oc rollout status deployment hello-world1 -n project-01 --timeout=120s
oc rollout status deployment hello-world2 -n project-01 --timeout=120s

# Verify — should show OSSM 3 istiod
istioctl ps -n project-01 | grep -E "hello-world|istiod"
...
#### project-02 (hello-world3, hello-world4)

```bash
oc rollout restart deployment -n project-02
oc rollout status deployment hello-world3 -n project-02 --timeout=120s
oc rollout status deployment hello-world4 -n project-02 --timeout=120s

istioctl ps -n project-02 | grep -E "hello-world|istiod"
curl -sk https://hello-world3.devops.local/ | head -5
curl -sk https://hello-world4.devops.local/ | head -5
```

#### project-03 (hello-world5, hello-world6)

```bash
oc rollout restart deployment -n project-03
oc rollout status deployment hello-world5 -n project-03 --timeout=120s
oc rollout status deployment hello-world6 -n project-03 --timeout=120s

istioctl ps -n project-03 | grep -E "hello-world|istiod"
curl -sk https://hello-world5.devops.local/ | head -5
curl -sk https://hello-world6.devops.local/ | head -5
```

#### project-04 (hello-world7, hello-world8)

```bash
oc rollout restart deployment -n project-04
oc rollout status deployment hello-world7 -n project-04 --timeout=120s
oc rollout status deployment hello-world8 -n project-04 --timeout=120s

istioctl ps -n project-04 | grep -E "hello-world|istiod"
...
```bash
oc rollout restart deployment -n devops
oc rollout status deployment -n devops --timeout=120s

istioctl ps -n devops | head -10
```

### 9.3 Verify All Workloads Are on OSSM 3
...
istioctl ps --istioNamespace istio-system | grep -E "hello-world|istio-ingressgateway"
```
...
istioctl ps --istioNamespace istio-system
...
spec:
  ...
  podSelector:
    matchLabels:
      app: istiod
      istio.io/rev: default   # Tag name — matches all revisions the tag points to
  policyTypes:
  - Ingress
```
...
# (no --revision flag needed — using tag name "default" for all namespaces)
```
...
helm upgrade cert-manager-istio-csr jetstack/cert-manager-istio-csr \
  ...
  --namespace istio-system \
  --wait \
  --set "app.istio.revisions={basic,default}" \
